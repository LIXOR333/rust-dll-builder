...
public:
	...
	static void RenderExplosions() {
		for (int i = 0; i < LogSystem::loggedExplosions.size(); i++) {
			...
			//	OxyRender::Text(explPos,D2D1::ColorF::Red,			true,true,StringConverter::ToUnicode(StringFormat::format(xorstr_("%s [%1.0fm] [%d]"),explosion.name.c_str(),Math::Distance_3D(explosion.position, LocalPlayer::Entity()->get_bone_pos(head)),(int)(timee - (get_time_since_startup() - LogSystem::loggedExplosions[i].timeSince)))).c_str()

				//);
			
				std::string string;
				char buf[256] = { 0 }; sprintf_s(buf, xorstr_("%s [%1.0fm] [%d]"), explosion.name.c_str(), Math::Distance_3D(explosion.position, LocalPlayer::Entity()->get_bone_pos(head)), (int)(timee - (get_time_since_startup() - LogSystem::loggedExplosions[i].timeSince)));
				string += buf;
				g_Render->DrawString(explPos.x, explPos.y, ImColor(255, 255, 255), render2::outline | render2::centered_x, calibri_, 12, string.c_str());
			}
		}
	}
};
void LogSystem::draw_text(Vector2 pos, std::string str, ImColor col, int timer) {
	g_Render->DrawString(pos.x + 5, pos.y + 5, ImColor(255, 255, 255), render2::outline, calibri_, 12, str.c_str());
}
