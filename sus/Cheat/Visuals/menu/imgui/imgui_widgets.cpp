...
bool ImGui::ButtonEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImU32 outside_solid_col = ImColor(25, 25, 25);
    const ImU32 inside_hover_col = ImColor(accent_color[0], accent_color[1], accent_color[2], (int)it_anim->second.closing_alpha);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetWindowSize().x - style.WindowPadding.x - 10;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 7));
    const ImRect frame_bb2(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb2.Min, frame_bb2.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, -8.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;

 
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb2))
        return false;
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(35, 35, 35), 4);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(40, 40, 40), 4);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Max.x + 2, frame_bb.Max.y), ImColor(accent_color[0], accent_color[1], accent_color[2]), 4);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    auto buf_size = fMain->CalcTextSizeA(15, FLT_MAX, 0, value_buf);
    if (hovered)
    {
        /*window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - buf_size.x / 3, frame_bb2.Min.y + 5), ImVec2(grab_bb.Max.x + buf_size.x / 3, frame_bb2.Min.y + 3 + buf_size.y), ImColor(35, 35, 35, 255 * 0.9), 4);
        window->DrawList->AddRect(ImVec2(grab_bb.Min.x - buf_size.x / 3, frame_bb2.Min.y + 5), ImVec2(grab_bb.Max.x + buf_size.x / 3, frame_bb2.Min.y + 3 + buf_size.y), ImColor(40, 40, 40, 255 * 0.9), 4);*/
        /*RenderTextClipped(ImVec2(grab_bb.Min.x - buf_size.x / 3 + 1, frame_bb2.Min.y + 4), ImVec2(grab_bb.Max.x + buf_size.x / 3, frame_bb2.Min.y + 3 + buf_size.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.0f));*/
    }
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 16), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return value_changed;
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags | ImGuiColorEditFlags_AlphaPreview))
        {
        ...
        if (BeginPopup("picker", ImGuiWindowFlags_NoMove))
        {
            ...
            ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_AlphaBar;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        RenderFrame(bb.Min, bb.Max, ImColor(accent_color[0], accent_color[1], accent_color[2]), false, 0.0f);
    }
